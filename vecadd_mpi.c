...
// Iterating MPI recieves for example is inefficient but easy to understand.
...
int main(int argc, char *argv[]) 
{
  ...
  if(pid < np - 1){
    ...
    fflush(stdout);
    if(gpu)
      compute_node_gpu(vector_size / (np - 1));
    else
